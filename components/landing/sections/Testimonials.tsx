  ...
  return (
    <section id="testimonials" className="relative py-20 md:py-32 bg-black overflow-hidden">
      ...
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        <div className="mb-12 md:mb-20 text-right">
          <Reveal>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 mb-6 backdrop-blur-md">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-emerald-400 font-mono text-[10px] md:text-xs tracking-wider uppercase">شركاء النجاح</span>
            </div>
            ...
            <p className="text-gray-300 text-base md:text-lg max-w-2xl ml-auto mr-0 font-normal leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {[
            ...
            <Reveal key={i} delay={i * 0.2}>
              <SpotlightCard className="p-6 md:p-8 rounded-[2rem] flex flex-col justify-between h-full bg-white/[0.03] border-white/10 hover:border-emerald-500/30 transition-all group">
                <div className="mb-6 md:mb-8">
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map(s => <Star key={s} className="h-3.5 w-3.5 fill-emerald-500 text-emerald-500" />)}
                    </div>
                    <Quote className="h-6 w-6 md:h-8 md:w-8 text-white/10 group-hover:text-emerald-500/20 transition-colors" />
                  </div>
                  <p className="text-gray-200 leading-relaxed text-base md:text-lg font-normal">"{t.text}"</p>
                </div>
                ...
                <div className="flex items-center gap-4 pt-6 border-t border-white/5">
                  ...
                  <div>
                    <div className="font-bold text-white text-sm md:text-base">{t.author}</div>
